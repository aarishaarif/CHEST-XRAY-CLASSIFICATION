...
def find_last_convolutional_layer(model: keras.Model) -> keras.layers.Conv2D:
    """Find the deepest Conv2D layer, including inside nested models."""
    ...
    def search_layers(layers):
        for layer in reversed(layers):
            ...
            if isinstance(layer, keras.Model):
                found = search_layers(layer.layers)
                if found is not None:
                    return found

        return None

    convolutional_layer = search_layers(model.layers)

    if convolutional_layer is None:
        raise GradCAMError(
            "The loaded model has no accessible Conv2D layer for Grad-CAM."
        )
